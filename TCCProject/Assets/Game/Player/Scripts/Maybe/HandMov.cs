...
public class HandMov : MonoBehaviour
    ...
    void Move()
    {
        ...
        // Verifica se o alvo está à direita ou à esquerda do objeto
        float dot = Vector2.Dot(targetDirection, transform.right);
        float newAvoidanceForce = (dot > 0) ? Mathf.Abs(avoidanceForce) : -Mathf.Abs(avoidanceForce);

        avoidanceDirection *= newAvoidanceForce;
